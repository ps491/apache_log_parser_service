...
from aggregator.tasks import process_parse_log_task


@receiver(post_save, sender=LogFile)
def item_signal(sender, instance: LogFile, created, **kwargs):
    """Создание записи файла, делается для того, чтобы была возможность также добавлять или удалять вручную из админки логи"""
    ...
    if created:
        ...
        process_parse_log_task(instance.id)
    return True
